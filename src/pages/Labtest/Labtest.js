...
function LabTest({ userRole }) {
  ...
  const hendleEditLab = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `https://hospital-mangment.onrender.com/lab/${eidtLabId}`,
        {
          method: "PUT",
          body: JSON.stringify(lab),
          headers: {
    ...
    <Wrapper>
      ...
      <Content>
        ...
        {loading === true ? (
          ...
          <Activity>
            <Link to="/labtest/1">
              <GreenButton>List report</GreenButton>
            </Link>
            <TextInput
              name="patientId"
              defaultValue={lab.patientId}
            ...
            <TextInput
              name="doctorName"
              defaultValue={lab.doctorName}
              onChange={hendleChange}
              type="radio"
              title="Doctor Name"
              options={[{ label: "Robiul", value: "Robiul" }]}
            />
            <TextInput
              name="status"
            ...
            />
            <GreenButton onClick={editMode ? hendleEditLab : hendleSaveLabTest}>
              Save
            </GreenButton>
          </Activity>
        )}
