...
function AddMedicine({ userRole }) {
  const { hendleMedicineUI, editMedicineId, medicineList } =
  ...
  useEffect(() => {
    if (editMedicineId && medicineList) {
      setEditMode(true);
      const editMedicine = medicineList.find(
        (item) => item._id === editMedicineId
      );
      setMedicine((prvMedicine) => ({
        ...prvMedicine,
  ...
  const hendleEditMedicine = async () => {
    try {
      const response = await fetch(
        `https://hospital-mangment.onrender.com/medicine/${editMedicineId}`,
        {
          method: "PUT",
    ...
    <Wrapper>
      ...
      <Content>
        ...
        {loading ? (
          ...
          <Activity>
            <Link to="/medicine/1">
              <GreenButton>List Medicine</GreenButton>
            </Link>
            <TextInput
            ...
            />
            <TextInput
              defaultValue={medicine.manufactured}
              onChange={hendleChange}
              name="manufactured"
            ...
            />
            <GreenButton
              onClick={editMode ? hendleEditMedicine : hendleSaveMedicine}
            >
              Save
            </GreenButton>
          </Activity>
