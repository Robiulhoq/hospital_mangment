...
function ListSchedule({ userRole }) {
  const { doctorList, hendleDoctorUI, hendleEditSchedule } =
  ...
  const hendleDeleteSchedule = async (doctorId, scheduleId) => {
    try {
      const response = await axios.delete(
        `https://hospital-mangment.onrender.com/doctor/schedule/${doctorId}/${scheduleId}`,
        {
    ...
    <Wrapper>
      ...
      <Content>
        ...
        <Activity>
          <DataFiltter>
            <Link to="/schedule/0">
              <GreenButton>+ Add Schedule</GreenButton>
            </Link>
            <div>
                ...
                item.schedule.map((sche) => (
                  <tr key={index}>
                    ...
                    <td>
                      ...
                      <Link to="/schedule/0">
                        <BiEdit
                          onClick={() => hendleEditSchedule(item._id, sche._id)}
                          size="1.5rem"
                          color="darkblue"
                        />{" "}
                      </Link>
                      <AiFillDelete
                        onClick={() => hendleDeleteSchedule(item._id, sche._id)}
                        color="red"
                        size="1.5rem"
                      />
                    </td>
