...
function Finance({ userRole }) {
  ...
  const dabitAccount = accountList.filter(
    (item) => item.accountType === "Dabit"
  );
  const defaultAccountName =
    dabitAccount.length > 0 ? dabitAccount[0].accountName : "";
  ...
  const handleInvoiceChange = (e, index) => {
    ...
    updatedInvoice[index].subTotal =
      updatedInvoice[index].quantity * updatedInvoice[index].price;
  ...
  const grandTotal =
    total +
    (fullInvoice.vat / 100) * total -
    (fullInvoice.discount / 100) * total;
  ...
  useEffect(() => {
    if (fullInvoice.patientId.length == 24) {
      fetch(
        `https://hospital-mangment.onrender.com/patient/filter/${fullInvoice.patientId}`,
        {
        ...
        {loading == true ? (
          ...
          <Activity id="invoice_container">
            <section id="patientAndHospital">
              <div className="patient_info_container">
                ...
                <TextInput
                  className="custom_input"
                  value={
                    singlePatient.length ? singlePatient[0].fastName : null
                  }
                  title="Full Name"
                  type="text"
                />
                <TextInput
                  className="custom_input"
                  value={
                    singlePatient.length && fullInvoice.patientId
                      ? singlePatient[0].address
                      : null
                  }
                  title="Address"
                  type="text"
                ...
                {invoice.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ display: "flex" }}>
                      <GreenButton onClick={handleAddInvoice}>Add</GreenButton>
                      <BlueButton onClick={handleRemoveInvoice}>
                        Remove
                      </BlueButton>
                    </td>
                  </tr>
            ...
            <section id="save_and_activity">
              <div>
                <BlueButton>Reset</BlueButton>{" "}
                <GreenButton onClick={hendleSaveInvoice}>Save</GreenButton>
              </div>
            </section>
          </Activity>
        )}
