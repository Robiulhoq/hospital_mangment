...
function AddPayment({ userRole }) {
  const { accountList, hendlePaymentUI, editPaymentId, paymentList } =
    useContext(DataContext);
  const creditAccount = accountList.filter(
    (item) => item.accountType === "Credit"
  );
  ...
  useEffect(() => {
    if (editPaymentId && paymentList) {
      setEditMode(true);
      const editPayment = paymentList.find(
        (item) => item._id === editPaymentId
      );
      setPayment((prvPayment) => ({
        ...prvPayment,
  ...
  const hendleEditPayment = async () => {
    try {
      setLoading(true);
      const response = await fetch(
        `https://hospital-mangment.onrender.com/payment/${editPaymentId}`,
        {
          method: "PUT",
          body: JSON.stringify(payment),
          headers: {
    ...
    <Wrapper>
      ...
      <Content>
        ...
        {loading ? (
          ...
          <Activity>
            <Link to="/finance/5">
              <GreenButton>List Payment</GreenButton>
            </Link>
            <TextInput
            ...
            />
            <GreenButton
              onClick={editMode ? hendleEditPayment : hendleSavePayment}
            >
              Save
            </GreenButton>
          </Activity>
