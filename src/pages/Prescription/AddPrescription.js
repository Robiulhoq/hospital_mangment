  ...
  const hendleSavePrescription = async () => {
    try {
      ...
      const response = await axios.post(
        `https://hospital-mangment.onrender.com/prescription/${patientId}`,
        prescription,
        {
          headers: {
  ...
  return (
    <Wrapper>
      ...
      <Content>
        ...
        <Link to="/prescription/3">
          <GreenButton>Print Prescription</GreenButton>
        </Link>
        {loading ? (
          <Loading />
        ) : (
          <Activity id="invoice_container">
            <section id="patientAndHospital">
              <div className="patient_info_container">
                ...
                <TextInput
                  className="custom_input"
                  defaultValue={
                    singlePatient ? singlePatient[0].fastName : null
                  }
                  title="Full Name"
                  type="text"
                />
                <TextInput
                  className="custom_input"
                  defaultValue={singlePatient ? singlePatient[0].sex : null}
                  title="Sex"
                  type="text"
                />
                <TextInput
                  className="custom_input"
                  defaultValue={
                    singlePatient ? singlePatient[0].dathOfBirth : null
                  }
                  title="Dath of birth"
                  type="text"
                />
                ...
                {medicine.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ display: "flex" }}>
                      ...
                      <BlueButton onClick={handleRemoveMedicine}>
                        Remove
                      </BlueButton>
                    </td>
                  </tr>
              ...
              <table className="invoice_table diagnosis_headline">
                ...
                {diagnosis.map((item, index) => (
                  <tr key={index}>
                    ...
                    <td style={{ display: "flex" }}>
                      <GreenButton onClick={handleAddDiagnosis}>
                        Add
                      </GreenButton>
                      <BlueButton onClick={handleRemoveDiagnosis}>
                        Remove
                      </BlueButton>
                    </td>
                  </tr>
            ...
            <section id="save_and_activity">
              <div>
                ...
                <GreenButton onClick={hendleSavePrescription}>Save</GreenButton>
              </div>
            </section>
          </Activity>
        )}
